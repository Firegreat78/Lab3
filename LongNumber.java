...
public final class LongNumber
{
    ...
    LongNumber(String str, int sign)
    {
        ...
        int zeros = 0;
        while (zeros < str.length() && str.charAt(zeros) == '0') ++zeros;
        this.str = str.substring(zeros);
        this.sign = zeros == str.length() ? 0 : Integer.signum(sign);
    }
    ...
    {
        return Compare(new LongNumber(other));
    }
    ...
    private static String UnsignedMultiply(String a, String b)
    {
        ...
        if (shorter.isEmpty() || shorter == "0") return "0";
        if (shorter == "1") return longer;
        if (shorter.length() == 1)
        {
